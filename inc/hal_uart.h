 ...
 */
hal_status_t uart_send(uart_t uart, uint8_t* data, uint16_t size, uint16_t timeout);
 ...
 * @note Implement in hal_uart.c
 */
hal_status_t uart_recv(uart_t uart, uint8_t* buff, uint16_t size, uint16_t timeout);
 ...
 * @note Implement in hal_uart.c
 */
hal_status_t uart_send_it(uart_t uart, uint8_t* data, uint16_t size, uint16_t timeout);
...
*/
hal_status_t uart_register_callback(uart_t uart, callback_t callback, uart_callback_src_t src);
#else
/**
 * UART send ISR
 * @note This function should be called from lower level (driver's) ISR in the hal_uart.c
 */
void uart_send_isr(uart_t uart, hal_status_t status);

/**
 * UART receive ISR
 * @note This function should be called from lower level (driver's) ISR in the hal_uart.c
 */
void uart_recv_isr(uart_t uart, hal_status_t status);
#endif
